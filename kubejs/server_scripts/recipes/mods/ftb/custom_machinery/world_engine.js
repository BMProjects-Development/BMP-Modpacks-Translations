...
function applyStructures(builder, defs) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    var def =
      typeof ref === "string"
        ? typeof FTBStructures$WorldEngine !== "undefined"
          ? FTBStructures$WorldEngine[ref]
          : null
        : ref;
...
ServerEvents.recipes(function (event) {
  addWorldEngineRecipes(event, [
    ...
    {
      id: "ftb:world_engine/gateway_scylla",
      machineId: "ftb:world_engine",
      duration: 240,
      structures: ["machine_block_upgrade", "enderium_upgrade"],
      energyPerTick: 32768,
      itemInputs: [
        { item: "avaritia:diamond_lattice", count: 2 },
        { item: "ars_nouveau:air_essence", count: 4 },
        { item: "ars_nouveau:water_essence", count: 4 },
        {
          item: "projecte:dark_matter",
          count: 1,
      ...
      itemOutputs: [
        {
          item: 'gateways:gate_pearl[gateways:gateway="gateways:scylla"]',
          count: 1,
        },
      ],
    },
...
function structureNames() {
  try {
    return Object.keys(FTBStructures$WorldEngine || {});
  } catch (_) {
    return [];
  }
}
...
ServerEvents.commandRegistry(function (event) {
  ...
  var STRUCTS = typeof FTBStructures$WorldEngine !== "undefined" ? FTBStructures$WorldEngine : {};
  var UPGRADE_KEYS = [];
  ...
  root = root.then(
    Commands.literal("reset").then(
      Commands.literal("all").executes(function (context) {
        ...
        Object.values(FTBStructures$WorldEngine).forEach((structDef, structName) => {
  ...
  Object.keys(FTBStructures$WorldEngine).forEach((structName) => {
    root = root.then(
      Commands.literal("reset").then(
        Commands.literal(structName).executes(function (context) {
          ...
          let base_level = player
            .getServer()
            ["getLevel(net.minecraft.resources.ResourceKey)"](baseDim);
          var structDef = FTBStructures$WorldEngine[structName];
          if (!structDef) {
            player.sendSystemMessage(
...
const WorldEngineStateMachine = {
  ...
  tryContinue: function (level, player) {
    ...
    if (!struct) return;
    var structDef = FTBStructures$WorldEngine[struct];
    if (!structDef) return;
