...
(Опционально) Блоки крафта просто заполняют пространство в процессоре, чтобы превратить его в сплошную прямоугольную призму, если у вас недостаточно
...
(Необходимо)Хранилища крафтаф имеют все стандартные размеры ячеек (1k, 4k, 16k, 64k, 256k). В них хранятся ингредиенты и
...
(Опционально) Сопроцессоры крафта заставляют систему чаще отправлять партии ингредиентов из <ItemLink id="pattern_provider" />, 
заставляя CPU тикать быстрее.
Это позволяет им успевать за машинами, которые работают быстро. Примером этого является поставщик паттернов, 
окружённый <ItemLink id="molecular_assembler" />, который может передавать ингредиенты быстрее, чем один сборщик может обрабатывать, 
и таким образом распределять партии ингредиентов между окружающими сборщиками.

Некоторые сложные рецепты имеют несколько этапов, которые могут выполняться параллельно, например, изготовление досок и книг одновременно для создания книжных полок.
В окне статуса крафта (открывается правым кликом по CPU или через иконку молотка в [терминале](terminals.md)), эти
этапы будут отображаться как "запланированные". Каждый дополнительный сопроцессор позволяет выполнять параллельно ещё один из этих этапов (и, соответственно, отображать его как "в процессе крафта").
Однако это не так важно, потому что обычно у вас будет больше сопроцессоров просто для скорости вставки, чем этапов в рецепте, которые могут выполняться параллельно.
...
(Опционально) На мониторе отображается работа, которую выполняет процессор в данный момент.
