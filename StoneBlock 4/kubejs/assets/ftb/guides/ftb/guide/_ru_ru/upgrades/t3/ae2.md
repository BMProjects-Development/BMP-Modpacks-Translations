---
navigation:
  title: Квантовый туннель
  position: 6
  parent: ftb:t3_upgrades.md
  icon: "ae2:quantum_ring"
item_ids:
  - ae2:quantum_ring
---
# <Color id="green">Улучшение "Квантовый туннель"</Color>

<ItemImage id="ae2:quantum_ring" scale="3" />

<Color id="aqua">"Квантовый туннель"</Color> второстепенное улучшение, которое разблокирует возможность автоматизировать крафт на <Color id="gold">мировом двигателе</Color>. 

Вам нужно будет установить квантовое кольцо, подключенное к Вашей МЭ системе, и быть уверенным, что чанк прогружается. Мировой двигатель уже имеет загрузчик чанков.

Для этого улучшения необходимо улучшение <Color id="red">"Усовершенствованный корпус механизма"</Color>, чтобы его рецепт разблокировался.

## <Color id="gold">Разблокируемые рецепты:</Color>

<ItemGrid>
  <ItemIcon id="advanced_ae:reaction_chamber" />
  <ItemIcon id="mekanism:elite_tier_installer" />
  <ItemIcon id="pipez:ultimate_upgrade" />
  <ItemIcon id="xp_synthesiser:xp_synthesiser" />
</ItemGrid>
