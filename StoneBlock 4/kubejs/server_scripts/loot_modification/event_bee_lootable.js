LootJS.lootTables((event) => {
  ...
  event
    .getLootTable("irons_spellbooks:entities/additional_evoker_loot")
    .firstPool()
    .addEntry("irregular_implements:spectre_sapling");
});
...
LootJS.lootTables((event) => {
  ...
  const stone = LootEntry.of("minecraft:stone").when((c) =>
    c.matchTool(
      ItemPredicate.custom((stack) => stack.getEnchantmentLevel("minecraft:silk_touch") > 0).or(
        ItemPredicate.hasEnchantment("minecraft:silk_touch")
      )
    )
  );

  const cobble = LootEntry.of("minecraft:cobblestone").when((c) => c.survivesExplosion());

  const loot = LootEntry.alternative(pebble, stone, cobble);

  event.getBlockTable("minecraft:stone").clear().firstPool().addEntry(loot);
  event.getBlockTable("minecraft:stone").print();
  console.log(ItemPredicate.custom((stack) => stack.hasEnchantment("minecraft:silk_touch", 1)));
});

LootJS.lootTables((event) => {
  global.SB4$CREATIVE_BLOCKS.forEach((block) => {
    try {
      let loot_table = event.getBlockTable(block).modifyEntry((entry) => {
