...
function cutscene_intro(player) {
  ...
  cutscene.setDurationSeconds(20).setCurve("CATMULLROM").setEasing("EASE_IN_OUT");
  ...
  cutscene
    ...
    .executeAtSecond(
      7,
      (player) => {
        player["teleportTo(net.minecraft.server.level.ServerLevel,double,double,double,float,float)"](
          player.level,
          -14.5,
          1,
          0,
...
function cutscene_advanced_machine_upgrade(player) {
  var cutscene = new Cutscene();
  cutscene.setDurationSeconds(15).setCurve("CATMULLROM").setEasing("LINEAR");

  cutscene.getPath().addSpiral(3.5, -25, -1.5, 8.4, 8.4, 0, 1, 360);

  WorldengineCutscene(player, cutscene);
}
function cutscene_dark_void_upgrade(player) {
  var cutscene = new Cutscene();
  cutscene.setDurationSeconds(15).setCurve("CATMULLROM").setEasing("EASE_IN_OUT");

  cutscene.getPath().addSpiral(3.5, -23, -1.5, 3, 3, 0, 1, 360);
...
function cutscene_chroniton_upgrade(player) {
  ...
  cutscene.setDurationSeconds(15).setCurve("CATMULLROM").setEasing("EASE_IN_OUT");
  cutscene.getPath().addSpin(3.5, -24, -1.5, 0, 360, 0, 360);
  WorldengineCutscene(player, cutscene);
}

function cutscene_enchanting_upgrade(player) {
  var cutscene = new Cutscene();
  cutscene.setDurationSeconds(15).setCurve("CATMULLROM").setEasing("EASE_IN_OUT").setTimeEasing("EASE_IN");
...
function cutscene_enderium_upgrade(player) {
  ...
  spin.setDurationSeconds(10).setCurve("CATMULLROM").setEasing("EASE_IN_OUT");

  spin.getPath().addSpin(3.5, -21, -1.5, 0, 360, 0, 360);

  var cutscene = new Cutscene();
  cutscene.setDurationSeconds(5).setCurve("CATMULLROM").setEasing("EASE_IN_OUT").setNext(spin);
  cutscene
    .getPath()
...
function cutscene_awakened_core_upgrade(player) {
  ...
  cutscene.getPath().addSpiral(3.5, -10, -1.5, 15, 15, 0, 1, 360);
...
function cutscene_euphonium_upgrade(player) {
  ...
  spin.getPath().addSpin(3.5, -26, -1.5, 0, 360, 31, 360);
...
function cutscene_infinity_upgrade(player) {
  ...
  orbit.setDurationSeconds(15).setCurve("CATMULLROM").setEasing("EASE_IN_OUT");

  orbit.getPath().addSpiral(3.5, -26, -1.5, 3, 3, 0, 1, 360);

  var cutscene = new Cutscene();
  cutscene.setDurationSeconds(5).setCurve("CATMULLROM").setEasing("EASE_IN_OUT").setNext(orbit);
  cutscene
    .getPath()
    .addPoint(3.5, -18.82, 5.5, 180, -16.2, 0)
...
function cutscene_spirit_upgrade(player) {
  var orbit = new Cutscene();
  orbit.setDurationSeconds(13).setCurve("CATMULLROM").setEasing("EASE_IN_OUT");

  orbit.getPath().addSpiral(3.5, -17, -1.5, 7, 7, 0, 1, 360);

  var cutscene = new Cutscene();
  cutscene.setDurationSeconds(2).setCurve("CATMULLROM").setEasing("EASE_IN_OUT").setNext(orbit);
  cutscene.getPath().addPoint(3.5, -18.82, 5.5, 180, -16.2, 0).addPoint(3.5, -17, 5.5, 180, 0, 0);
...
function cutscene_twilight_forest_upgrade(player) {
  ...
  orbit.getPath().addSpiral(3.5, -21, -1.5, 7, 7, 0, 1, 360);
