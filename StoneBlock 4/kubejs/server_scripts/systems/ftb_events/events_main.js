...
const defaultSettings = {
  ...
  timeCooldown: 18000, //time in ticks to check if a player has had an event recently
};
...
function eventSystem(event, forceEvent, server) {
  ...
  if (!forceEvent) {
    ...
    if (!chosenPlayer.stages.has("events_enabled") && !Teams.hasStage(chosenPlayer, "events_enabled")) return;
  ...
  console.log(
    "Event " +
      chosenEvent.name +
      " triggered for player " +
      chosenPlayer.username +
      " in " +
      (endTime - startTime) +
      "ms"
  );
